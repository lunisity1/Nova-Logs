...
public class SearchDateVoucher extends SubCommand {

    private final NovaLogs novaLogs;

    public SearchDateVoucher(final NovaLogs novaLogs) {
        super("voucher-date");
    ...
    public void execute(CommandContext commandContext) {
        ...
        if (commandContext.getLength() == 3) {
            this.novaLogs.getVoucherLogsManager().searchDate(player, target, commandContext.get(2), 1);
            return;
        }

        if (commandContext.getLength() == 4) {
            this.novaLogs.getVoucherLogsManager().searchDate(player, target, commandContext.get(2), commandContext.asInt(3));
            return;
        }
