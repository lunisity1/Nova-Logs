...
public class SearchDateChat extends SubCommand {

    private final NovaLogs novaLogs;
// searchlogs chat-date <player> <date> <page>
    public SearchDateChat(final NovaLogs novaLogs) {
        super("chat-date");
    ...
    public void execute(CommandContext commandContext) {
        ...
        if (commandContext.getLength() == 3) {
            this.novaLogs.getChatLogsManager().searchDate(player, target, commandContext.get(2), 1);
            return;
        }

        if (commandContext.getLength() == 4) {
            this.novaLogs.getChatLogsManager().searchDate(player, target, commandContext.get(2), commandContext.asInt(3));
            return;
        }
